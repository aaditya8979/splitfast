...
import {
  ...
  CommandInput,
  CommandItem,
  CommandList, // Added CommandList for safety
...
export function CategorySelector({
  ...
  // SAFETY CHECK 1: Handle empty categories list
  const selectedCategory =
    categories.find((category) => category.id === value) ?? categories[0]

  // SAFETY CHECK 2: If still undefined, show loading or placeholder
  if (!selectedCategory) {
    return (
...
function CategoryLabel({ category }: { category: Category }) {
  const t = useTranslations('Categories')
  // 🟢 FIX: Return nothing if category doesn't exist (prevents crash)
